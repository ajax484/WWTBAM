...
const shuffleFn = (optionA, optionB) =>
  Math.random() * (optionB.weight + optionA.weight) - optionA.weight;
  ...
  icon.addEventListener("click", function () {
    ...
    if (icon.id === "phone") {
      ...
      setTimeout(() => {
        ...
        displayMessage3(
          `Friend suggests: ${reply.option}, reliability: ${reply.reliability}`,
          false
        );
      }, 3000);
    }
    ...
    if (icon.id === "fiftyfifty") {
      setTimeout(() => {
        displayMessage3(
          "removing two options and leaving the right one and one wrong one",
          false
        );
      }, 1500);
...
class Quiz {
  ...
  initializeQuestions() {
    ...
    queries.forEach((diff) => {
      console.log(
        `https://opentdb.com/api.php?amount=5&difficulty=easy&type=${diff}`
      );
      fetch(
        `https://opentdb.com/api.php?amount=5&difficulty=${diff}&type=multiple`
      )
        .then((res) => res.json())
        .then((res) => {
  ...
  fiftyFiftyProbSelect(options) {
    ...
    let newOptions = options
      ...
      .filter((option) => option.innerHTML !== this.answer)
      .filter((option) => option.innerHTML !== "dummy");
    ...
    if (option.innerHTML === this.answer) {
      ...
      wrongAnswer(
        ...
        Array.from(options).filter(
          (option) => option.innerHTML === this.answer
        ),
        this.questionIndex
      );
      return false;
...
function displayMessage1(type, answer) {
  infoBar.innerHTML += `
        <div class= "message row" >
            <p class="col-2 text-capitalize text-${
              type == "correct"
                ? "success"
                : type == "wrong"
                ? "danger"
            ...
            <p class="col-10 text-left">${
              type == "correct"
                ...
                : type == "wrong"
                ? "Whoops, that isn't right, the answer is "
                : "warning"
            } ${answer}</p>
        </div> 
    `;
...
function correctAnswer(params) {
  levelIndicatorTl
    .addLabel("start")
    .to(params, 0.5, { backgroundColor: "#28a745" }, "start")
    .fromTo(
      params,
      0.5,
      { backgroundColor: "#28a745" },
      { backgroundColor: "#E0A747", repeat: 5, yoyo: true, repeatDelay: 0.5 },
      "start+=.5"
    )
    ...
    .to(
      ...
      1,
      {
        backgroundColor: "#1724a6",
        onComplete: function () {
          levelIndicatorTl.set(params, { clearProps: "backgroundColor" });
        },
      },
      "start+=6"
    )
    ...
    .to(
      levelIndicator,
      1,
      { top: `${levels[x++].offsetTop - 3}`, ease: "power3.out" },
      "start+=2.5"
    )
    .fromTo(
      levelIndicator.querySelector(".level-indicator__inner"),
      0.75,
      { backgroundColor: "#1724a6" },
      { backgroundColor: "#e7bc73", repeat: 5, repeatDelay: 0.25, yoyo: true },
      "start+=3.5"
    )
    .to(
      levelIndicator.querySelector(".level-indicator__inner"),
...
function wrongAnswer(params1, params2, x) {
  levelIndicatorTl
    .addLabel("start")
    .call(toggle)
    .to(params2, 0.5, { backgroundColor: "#28a745" }, "start")
    .fromTo(
      params2,
      0.5,
      { backgroundColor: "#28a745" },
      { backgroundColor: "#E0A747", repeat: -1, yoyo: true, repeatDelay: 0.5 },
      "start+=.5"
    )
    .to(params1, 0.5, { backgroundColor: "#dc3545" }, "start")
    .call(checkToggle, null, "start+=6 ")
    .to(
      levelIndicator,
      1,
      { top: `${levels[getSafe(x)].offsetTop - 3}`, ease: "power3.out" },
      "start+=2.5"
    )
    .to(
      [levelIndicator, levelIndicator.querySelector(".level-indicator__inner")],
      1,
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  tl.addLabel("start")
    ...
    .to(
      levelIndicator,
      5,
      { top: `${levels[14].offsetTop - 3}`, onStart: () => disableLinks(true) },
      "start+=1"
    )
    .to(
      levelIndicator,
